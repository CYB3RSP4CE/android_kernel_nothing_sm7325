...
if [ -f "$kernel" ] && [ -d "$dts_dir" ]; then
	...
	cp out/modules/lib/modules/5.4.*/modules.* AnyKernel3/modules/vendor/lib/modules/5.4.287-NetHunter/
	sed -i 's/\(kernel\/[^: ]*\/\)\([^: ]*\.ko\)/\/vendor\/lib\/modules\/\2/g' AnyKernel3/modules/vendor/lib/modules/5.4.287-NetHunter/modules.dep
